...
Answer the visitor's latest question directly in 2-4 concise sentences, then suggest one practical next step when useful.
...
def build_prompt(message: str, workflow: str, history: list[dict[str, str]]) -> str:
...
def run_model(message: str, workflow: str, history: list[dict[str, str]]) -> str:
    prompt = build_prompt(message, workflow, history)
    env = os.environ.copy()
    env["HERMES_EPHEMERAL_SYSTEM_PROMPT"] = SYSTEM_RULES
    completed = subprocess.run(
        [
            "hermes", "chat", "-Q", "--safe-mode", "--ignore-rules",
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:
        ...
        try:
            answer = run_model(message, str(workflow), clean_history(payload.get("history")))
